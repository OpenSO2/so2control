...
int fixExposureTime(sParameterStruct * sSO2Parameters)
{
	...
	if (exposureTime <= 83560) {	//===========ELECTRONIC=SHUTTER==========
...
int camera_setExposureSwitch(sParameterStruct * sSO2Parameters, int timeSwitch)
{
	...
	case 1:
		logMessage("Camera is set to frameblanking mode.");
...
int setFrameBlanking(sParameterStruct * sSO2Parameters)
{
	...
	while (timeSwitch != 0 && timeSwitch != 3) {
		...
		case 1:
			logMessage("Image is underexposed, FB-value is set up");
...
int defaultConfig(sParameterStruct * sSO2Parameters)
{
	...
	if (PHX_OK != eStat) {
		...
		logMessage("Image depth recieved from camera is set to 12-bit");
	...
	if (PHX_OK != eStat) {
		...
		logMessage("Image file depth is set to 12-bit");
	...
	if (PHX_OK != eStat) {
		...
		logMessage("Right interpretation of FIFO overflows is set");
	...
	for (i = 0; i < 3; i++) {
		...
		if (PHX_OK == eStat) {
			...
			if (PHX_OK == eStat) {
				...
				logError("nothing was received from camera");
		}		// if ( PHX_OK == eStat )
		else
			logError("PHX_CommsTransmit(...) failed");
